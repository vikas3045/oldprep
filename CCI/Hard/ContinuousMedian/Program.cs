...
namespace ContinuousMedian
{
    class Program
    {
        ...
        {
            Heap lowers = new Heap(10, Heap.Type.Max);
            Heap highers = new Heap(10, Heap.Type.Min);

            foreach (int i in arr)
            {
                AddNumber(i, lowers, highers);
                Console.WriteLine("Current median: {0}", FindMedian(lowers, highers));
            }
        }

        private int FindMedian(Heap lowers, Heap highers)
        {
        ...
        private void AddNumber(int num, Heap lowers, Heap highers)
        {
            if (lowers.count == 0)
            {
                lowers.Insert(num);
        ...
        private void RebalanceHeaps(Heap lowers, Heap highers)
        {
            if (Math.Abs(lowers.count - highers.count) > 1)
            {
                Heap bigger; Heap smaller;
                if (lowers.count > highers.count)
                {
